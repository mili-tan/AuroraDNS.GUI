...
## 使用与测试
使用前请关闭同类软件以防冲突（如 DNSCrypt ）  

**普通用户指导**    
> 如果您是有命令行使用经验的，请看：高级用户指导  


1. 打开本软件之后，打开开关（开关在软件左上角)     
2. 然后下面三个圆形按钮都打开  
...
```
如果有解析，即代表本软件生效了。如果无效，请排除一切可能情况之后提 Issue    
然后你只需要检查 DNS 是否已设置为 `127.0.0.1`    

 - 查看所有网卡的网络设置参数
```cmd
ipconfig /all
```
 **注意：**你用的网卡 DNS 要为 `127.0.0.1` 才生效）  
 如果没有设置，请参考[此文章](https://jingyan.baidu.com/article/2fb0ba40833b0a00f2ec5f28.html)将第一个 DNS 设置为 `127.0.0.1`  第二个 DNS 推荐设置为 `腾讯的119或者运营商给你的 DNS`    
